...
def parse_market_start_time(question: str, reference_year: int = 2026) -> Optional[datetime]:
    ...
    try:
        ...
        # Build ET time then convert to UTC (+4 hours)
        et_dt = datetime(reference_year, month, day, hour_24, m1, 0)
        utc_dt = et_dt + timedelta(hours=4)
...
def parse_market_times(question: str, reference_year: int = 2026) -> tuple[Optional[datetime], Optional[datetime]]:
    ...
    try:
        ...
        start_et = datetime(reference_year, month, day, start_h, m1, 0)
        end_et = datetime(reference_year, month, day, end_h, m2, 0)

        start_utc = (start_et + timedelta(hours=4)).replace(tzinfo=timezone.utc)
        end_utc = (end_et + timedelta(hours=4)).replace(tzinfo=timezone.utc)
