...
Covers:
- Default 5m policy: entry_before_start_sec=45, entry_after_start_sec=240
- Default 15m policy: entry_before_start_sec=180, entry_after_start_sec=600
...
def test_5m_46s_before_start_fails():
    """Test 4: 601s before start is outside the 600s pre-window → too early."""
    now = at_offset(START_5M_UTC, -601)
    result = check_entry_window(Q_5M, now_utc=now)
...
def test_5m_91s_after_start_passes():
    """Test 6: 91s after start is within 240s post-window → passes."""
    now = at_offset(START_5M_UTC, 91)
    result = check_entry_window(Q_5M, now_utc=now)
    assert result.passed is True

def test_5m_241s_after_start_fails():
    """Test 6b: 601s after start exceeds 600s post-window → too late."""
    now = at_offset(START_5M_UTC, 601)
    result = check_entry_window(Q_5M, now_utc=now)
    assert result.passed is False
...
# ── Test 7: 15m at 179s before start → passes ────────────────────────────────

def test_15m_179s_before_start_passes():
    """Test 7: 179s before start is within the 180s pre-window → should pass."""
...
# ── Test 8: 15m at 181s before start → fails ─────────────────────────────────

def test_15m_181s_before_start_fails():
    """Test 8: 901s before start is outside the 900s pre-window → too early."""
    now = at_offset(START_15M_UTC, -901)
    result = check_entry_window(Q_15M, now_utc=now)
    assert result.passed is False
    assert result.rejection == EntryWindowRejection.TOO_EARLY_FOR_ENTRY_WINDOW


# ── Test 9: 15m at 599s after start → passes ─────────────────────────────────

def test_15m_599s_after_start_passes():
    """Test 9: 599s after start is within the 600s post-window → should pass."""
    now = at_offset(START_15M_UTC, 599)
    result = check_entry_window(Q_15M, now_utc=now)
    assert result.passed is True, f"Expected pass, got: {result.reason}"


# ── Test 10: 15m at 601s after start → fails ─────────────────────────────────

def test_15m_601s_after_start_fails():
    """Test 10: 901s after start exceeds the 900s post-window → too late."""
    now = at_offset(START_15M_UTC, 901)
    result = check_entry_window(Q_15M, now_utc=now)
    assert result.passed is False
    assert result.rejection == EntryWindowRejection.TOO_LATE_FOR_ENTRY_WINDOW
...
def test_5m_result_contains_correct_window_bounds():
    ...
    assert result.passed is True
    assert result.window_opens_at == START_5M_UTC - timedelta(seconds=600)
    assert result.window_closes_at == START_5M_UTC + timedelta(seconds=600)
...
def test_15m_result_contains_correct_window_bounds():
    ...
    assert result.passed is True
    assert result.window_opens_at == START_15M_UTC - timedelta(seconds=900)
    assert result.window_closes_at == START_15M_UTC + timedelta(seconds=900)
...
def test_horizon_above_20_returns_unavailable():
    """Horizon > 65m has no defined policy → ENTRY_WINDOW_UNAVAILABLE."""
    q_long = "Bitcoin Up or Down - March 16, 7:00PM-9:00PM ET"  # 2h market
...
def test_default_policy_5m_values():
    """Default 5m policy must be 600s / 600s (widened)."""
...
def test_default_policy_15m_values():
    """Default 15m policy must be 900s / 900s (widened)."""
