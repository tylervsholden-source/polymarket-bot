# 174. Tur Strateji İncelemesi — 2026-09-24
...
## Bu turda yapılanlar
- Açık PR kontrolü: round-173'ün PR'ı (#275, "docs: 173rd daily strategy
  ...
  (merge commit `cba2d11`). Bu oturumun branch'i `origin/main` üzerine
...
ait test verisi (ör. Oscar/spor/e-spor marketleri — `docs/architecture.md`
gereği kapsam dışı, `mr_type: extreme` alanı devre dışı mean-reversion
stratejisine ait) — canlı hesabı yansıtmıyor, round-169–173'te not düşülen
bulguyla tutarlı.
...
bulgusu kullanıcıya bildirim olarak iletilmişti. Round-159–173 aynı
bulguyu doğruladı, tekrar bildirim göndermedi — bu turda da hiçbir şey
değişmedi (round-173'ün PR'ı bu tur başlamadan kısa süre önce açılmıştı),
