...
class Orchestrator:
    def __init__(self):
        ...
        # Kapanan/işlem gören market_id'leri tekrar seçme (session + kalıcı)
        self._session_closed_markets: set = set()
        self._load_market_cooldowns()
    ...
    async def _cycle(self):
        ...
        for signal in signals:
            ...
            if self.position_manager.has_position(market_id):
                continue
            if market_id in self._session_closed_markets:
                continue
            ...
            if self._is_live_trading():
                ...
                # ── APPROVAL QUEUE: Direkt emir vermek yerine kuyruğa ekle ──
                _enqueue_order({
                    "market_id": market_id,
                ...
                logger.info(
                    f"ONAY BEKLİYOR: {market['question'][:50]} | "
    ...
    def _rate_limit_ok(self) -> bool:
        ...
        if len(self._order_timestamps) >= self._max_orders_per_hour:
            logger.warning(
                f"Rate limit: Son 1 saatte {len(self._order_timestamps)} emir verildi "
                f"(max {self._max_orders_per_hour}). Yeni emir engellendi."
            )
            return False
        return True
    ...
    async def _execute_approved_orders(self):
        ...
        for order_req in approved:
            ...
            if not self._rate_limit_ok():
                logger.warning(f"Rate limit aşıldı, emir bekletiliyor: {question[:50]}")
                break  # Kalan onaylı emirler sonraki döngüde denenecek
    ...
    async def _finalize_cycle(self, markets: list, candidates: list):
        ...
        self._session_closed_markets.update(newly_closed)
        for mid in newly_closed:
            self._save_market_cooldown(mid)
    ...
    _COOLDOWN_FILE = os.path.join(
        ...
        "data", "market_cooldowns.json",
    )

    def _load_market_cooldowns(self):
        """Başlangıçta kalıcı cooldown listesini yükle."""
        import json as _json
        try:
            with open(self._COOLDOWN_FILE) as f:
                data = _json.load(f)
            # Son 24 saat içindeki cooldown'ları yükle
            import time
            cutoff = time.time() - 86400
            for mid, ts in data.items():
                if ts > cutoff:
                    self._session_closed_markets.add(mid)
            logger.info(f"Kalıcı cooldown yüklendi: {len(self._session_closed_markets)} market.")
        except (FileNotFoundError, Exception):
            pass

    def _save_market_cooldown(self, market_id: str):
        """Yeni cooldown'u dosyaya yaz."""
        ...
        os.makedirs(os.path.dirname(self._COOLDOWN_FILE), exist_ok=True)
